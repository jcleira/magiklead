...
- [ ] Trigger the erasure flow on production: submit a request, click the link in the *real* inbox (not MailHog), confirm flow completes successfully. The email arrives in the inbox (not spam) — definitive spam check is in [issue #10](./10-sender-domain-auth.md).
...
- Sender domain DNS records (SPF/DKIM/DMARC) — see [issue #10](./10-sender-domain-auth.md). Resend's dashboard generates the DKIM record but adding it to DNS belongs to the next slice.
